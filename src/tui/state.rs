...
impl AppState {
    ...
    pub fn toggle_tag(&mut self) -> Result<()> {
        ...
        // Update current_tags on the popup to reflect the toggle
        if let Some(ref mut input) = self.tag_input {
            if is_applied {
                input.current_tags.retain(|t| t != &tag);
            } else {
                input.current_tags.push(tag.clone());
                input.current_tags.sort();
            }
            // Also update all_tags if a new tag was just created
            if !input.all_tags.contains(&tag) {
                input.all_tags.push(tag);
                input.all_tags.sort();
            }
            // Clear input and rebuild filter list, but preserve selection when browsing
            let had_input = !input.input.is_empty();
            input.input.clear();
            let prev_index = input.selected_index;
            input.update_filter();
            if !had_input {
                // Browsing mode: keep cursor where it was (clamped to list bounds)
                input.selected_index = prev_index.min(
                    input.filtered_tags.len().saturating_sub(1),
                );
            }
        }
